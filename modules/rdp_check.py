...
class RDPCHECK:
    def __init__(self, username='', password='', domain='', hashes=None):
        ...
        if hashes is not None:
            ...
            self.__lmhash = a2b_hex(self.__lmhash)
            self.__nthash = a2b_hex(self.__nthash)
        else:
            lmhash = ''
            nthash = ''

    def run(self, host, hashes = None):
        tpkt = TPKT()
        tpdu = TPDU()
        rdp_neg = RDP_NEG_REQ()
        ...
        s.settimeout(None) #Workaround for OpenSSL Issue where specifying a timeout = failure
        ...
        type3, exportedSessionKey = ntlm.getNTLMSSPType3(auth, ts_request['NegoData'], self.__username, self.__password, self.__domain,
                                                         self.__lmhash, self.__nthash, use_ntlmv2=True)
        ...
        # fields are omitted from the TSRequest structure.
        tsp = TSPasswordCreds()
        tsp['domainName'] = self.__domain
        tsp['userName'] = self.__username
        tsp['password'] = self.__password
        tsc = TSCredentials()
...
if __name__ == '__main__':
    ...
    executer = RDPCHECK(username, password, domain, options.hashes)
