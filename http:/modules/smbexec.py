...
class CMDEXEC:
    ...
    def run(self, remoteName, remoteHost):
        ...
        try:
            ...
            sys.stdout.flush()
                    # Error messages:
                    # bad creds (username, pwd, domain, hash):   SMB SessionError: STATUS_LOGON_FAILURE(The attempted logon is invalid. This is either due to a bad username or authentication information.)
                    # bad sharename:   SMB SessionError: STATUS_BAD_NETWORK_NAME({Network Name Not Found} The specified share name cannot be found on the remote server.)
                    # Insufficent permissions: failed DCERPC Runtime Error: code: 0x5 - rpc_s_access_denied
            if self.__mode == 'SERVER':
                serverThread.stop()
...
class RemoteShell(cmd.Cmd):
    def __init__(self, share, rpc, mode, serviceName):
        ...
        self.__scmr.connect()
        #except Exception as e:
            # reasons we hit this (scmr.connect fails instead of CMDEXEC.run try block for self.shell = RemoteShell failing
            # Error messages:
            # IP not reachable E= SessionError: STATUS_BAD_NETWORK_NAME({Network Name Not Found} The specified share name cannot be found on the remote server.)
            # Port closed: [Errno Connection error (172.66.10.3:446)] [Errno 111] Connection refused
            #print("failed %s" % e)
            #sys.stdout.flush()
            #logging.critical(str(e))
            #sys.exit(1)
