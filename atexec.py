...
class TSCH_EXEC:
    ...
    def doStuff(self, rpctransport):
        ...
        dce.connect() #fails if creds are wrong
    ...
    <Exec>
      ...
      <Arguments>/C %s &gt; %%windir%%\\Temp\\%s 2&gt;&amp;1</Arguments>
        ...
        try:
            ...
            tsch.hSchRpcRegisterTask(dce, '\\%s' % tmpName, xml, tsch.TASK_CREATE, NULL, tsch.TASK_LOGON_NONE) #fails if in sufficient permissions
            taskCreated = True # task created, this means shell/RCE is likely

            #logging.info('Running task \\%s' % tmpName)
            tsch.hSchRpcRun(dce, '\\%s' % tmpName) # Run the task!


            done = False
            while not done:
                #logging.debug('Calling SchRpcGetLastRunInfo for \\%s' % tmpName)
                resp = tsch.hSchRpcGetLastRunInfo(dce, '\\%s' % tmpName) # checks if running of the task was successful.
        ...
        #waitOnce = True
        '''while True: Commented out this block, it is used to retrieve the result of the command execution.
            try:
